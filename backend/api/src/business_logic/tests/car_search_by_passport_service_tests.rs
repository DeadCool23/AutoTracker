mod car_search_by_passport_service_tests {
    ...
    async fn test_handle_search_car_by_passport_success() {
        ...
        let res = service
            .search_cars_by_owner_passport(&DocumentMother::valid_document())
            .await;
    ...
    #[tokio::test]
    async fn test_handle_search_car_by_passport_invalid_passport() {
        let service = SearchService::from(Box::new(MockCarRepo), Box::new(MockTrackInfoRepo));

        let res = service
            .search_cars_by_owner_passport(&DocumentMother::invalid_document())
            .await;
