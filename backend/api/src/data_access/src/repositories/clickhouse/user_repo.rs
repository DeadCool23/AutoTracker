...
impl ClickHouseUserRepo {
    ...
    fn form_row_to_user_with_id(row: &ClickHouseUserRow) -> UserWithId {
        log::debug!("Converting database row to User");
        let passport_serial: Option<i32> = row.passport_serial;
        let passport_num: Option<i32> = row.passport_num;
    ...
    async fn get_row_by_id(&self, id: usize) -> Result<Option<ClickHouseUserRow>, DataAccessError> {
        log::info!("ClickHouse: Getting user by id: {}", id);

        let query = "SELECT * FROM AppUser WHERE id = ? LIMIT 1";
        log::debug!("Executing query: {}", query);
...
impl UserRepository for ClickHouseUserRepo {
    ...
    async fn get_user_with_id_by_auth_info(
        ...
        if row.len() == 1 {
            log::info!("User authenticated successfully: {}", email);
            Ok(Some(Self::form_row_to_user_with_id(&row[0])))
        } else {
            log::warn!("Authentication failed for user: {}", email);
            Ok(None)
        }
    ...
    async fn get_user_by_id(&self, id: usize) -> Result<Option<UserWithId>, DataAccessError> {
        ...
        let row = self.get_row_by_id(id).await?;
    ...
    async fn update_user_passport_by_id(
        ...
        if self.get_row_by_id(id).await?.is_none() {
            return Err(DataAccessError::ClickHouseBaseError(
                clickhouse::error::Error::Custom("User not found".to_string()),
            ));
        }
